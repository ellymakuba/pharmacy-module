...
function submitToFetchRecords(){
   $j('#west_panel_content').load("resources/subpages/FmapReport.form?startDate="+$j("#startDate").val()+"&endDate="+$j("#endDate").val(), function () {});
...
<div id="fmapReportDIV">
   ...
   <input type="button" onclick="tableToExcel('fmapReportTable', 'Export Data')" value="Export to Excel"/></br></br>
   <a href="generateExcel.form">Open Excel</a></br>
...
%>
<h1><a>FMAP Report Between <%=reportStartDate %> To <%=reportEndDate%></a></h1>
	<table id="fmapReportTable" class="visibleBorder">
		<thead>
			<tr>
			    <th>Regimen Code</th>
                <th>Regimen Name</th>
		...
		<tbody>
           ...
           %><tr>
             ...
             <td><%=pharmacyEncounterInstance.getRegimenName()%></td>
              <td><%=numberOfPatientsOnRegimen%></td>
              </tr>
           <%} %>
		</tbody>
	</table>
