...
function submitToFetchRecords(){
   $j('#west_panel_content').load("resources/subpages/D-CDRR.form?startDate="+$j("#startDate").val()+"&endDate="+$j("#endDate").val(), function () {});
...
<%
    ...
    Integer quantityOfStockTransfered=0;
    Integer quantityOfDrugWithLessThanSixMonthsToExpire=0;
    Integer quantityOfDrugsReceivedThroughTransfer=0;
    Integer quantityOfDrugsIssuedThroughTransfer=0;
    InventoryMetaData inventoryMetaData;
    Integer unitsInAPack=1;
    Integer quantityRequiredForResupply=0;
    Integer quantityOfDrugsReceivedThroughTransferTotal,quantityOfDrugsIssuedThroughTransferTotal;
%>

<div id="DCDRReportDIV">
   ...
   <input type="button" onclick="tableToExcel('DCDRReportTable', 'Export Data')" value="Export to Excel"/></br></br></br>
...
%>
<h1><a>DCDR Report For Central Sites Between <%=reportStartDate %> To <%=reportEndDate%></a></h1>
	<table id="DCDRReportTable" class="visibleBorder">
		<thead>
			<tr>
                ...
                <th>Total Quantity ISSUED to ARV dispensing sites (Satellite sites plus Central site dispensing point(s) where relevant)</th>
                <th>Losses (Damages,Expiries,Missing)</th>
                <th>Positive Adjustments(Borrowed from Other Facilities)</th>
                <th>Negative Adjustments(Issued out to Other Facilities)</th>
                <th>End of Month Physical Count</th>
                <th>Reported Aggregated Quantity CONSUMED in the reporting period (Satellite sites plus Central site dispensing point where relevant)</th>
                <th>Reported Aggregated Physical Stock on Hand at end of reporting period (Satellite sites plus Central site dispensing point where relevant)</th>
                <th colspan=2>Drugs with less than 6 months to expiry(Central site / District store)</th>
                <th>Days out of stock this Month at Central site/ District store</th>
		...
		<tbody>
           ...
           Drug drugFromContext = Context.getConceptService().getDrugByNameOrId(pharmacyStoreInstance.getDrugs().getId().toString());
                        ...
                        drugQuantityTransfered=0;
                        quantityOfStockTransfered=0;
                        quantityOfDrugsReceivedThroughTransfer=0;
                        quantityOfDrugsIssuedThroughTransfer=0;
                        quantityOfDrugsIssuedThroughTransferTotal=0;
                        quantityOfDrugsReceivedThroughTransfer=0;
                        quantityOfDrugsReceivedThroughTransferTotal=0;
                        unitsInAPack=1;
                        ...
                        if(service.getCumilativeDrugStockTransferedAtLocationBetweenTwoDates(startDate,endDate,pharmacyStoreInstance.getDrugs().getUuid(),pharmacyLocation.getUuid()) !=null){
                            quantityOfStockTransfered=service.getCumilativeDrugStockTransferedAtLocationBetweenTwoDates(startDate,endDate,pharmacyStoreInstance.getDrugs().getUuid(),pharmacyLocation.getUuid());
                        }
                        ...
                        for(DrugTransactions drugTransactionInstance:receivedDrugTransactionsList){
								...
								if(drugTransactionObjectReceived !=null){
									quantityOfDrugsReceivedThroughTransfer=drugTransactionObjectReceived.getQuantityIn();
									quantityOfDrugsReceivedThroughTransferTotal=quantityOfDrugsReceivedThroughTransferTotal+quantityOfDrugsReceivedThroughTransfer;
								}
                        }
                        for(DrugTransactions drugTransactionInstance:transferedDrugTransactions){
								...
								if(drugTransactionObjectTransfered !=null){
									quantityOfDrugsIssuedThroughTransfer=drugTransactionObjectTransfered.getQuantityIn();
									quantityOfDrugsIssuedThroughTransferTotal=quantityOfDrugsIssuedThroughTransferTotal+quantityOfDrugsIssuedThroughTransfer;
								}
                        }
                        ...
                        if(inventoryMetaData!=null){
                              unitsInAPack=inventoryMetaData.getUnitsInPack();
                        }
                        quantityOfDrugDispensed=quantityOfDrugDispensed/unitsInAPack;
                        quantityRequiredForResupply=((quantityOfDrugDispensed*2)-(pharmacyStoreInstance.getQuantity()/unitsInAPack));
                        %>
                       <tr>
                        ...
                        <%}else{%>
                        <td></td><%}%>
                        <td><%=openingStock/unitsInAPack%></td>
                        <td><%=stockReceivedForDrug/unitsInAPack%></td>
                        <td><%=quantityOfDrugDispensed%></td>
                        <td></td>
                        <td><%=quantityOfDrugsReceivedThroughTransfer/unitsInAPack%></td>
                        <td><%=quantityOfDrugsIssuedThroughTransfer/unitsInAPack%></td>
                        <td><%=pharmacyStoreInstance.getQuantity()/unitsInAPack%></td>
                        <td></td>
                        <td></td>
