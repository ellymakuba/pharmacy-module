...
var link;
var aData;
$j("#rfpCashierReportForm").validate(); //
$j("#cashierReportDateFrom").datepicker();
$j("#cashierReportDateTo").datepicker();
$j("form#rfpCashierReportForm").submit(function () {
    if ($j("#rfpCashierReportForm").valid()) {
        ooTable = $j('#cashierReportTable').dataTable({
            bJQueryUI:true,
            bRetrieve:true,
            bServerSide:true,
            bAutoWidth:false,
            bProcessing:true,
            sAjaxSource: "rfpCashierReportControllerRequest.form?" + "datef=" + $j('#cashierReportDateFrom').val() + "&datet=" + $j('#cashierReportDateTo').val(),
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var iTotal =[0,0,0,0,0,0,0];
                for (var i = 0; i < aaData.length; i++) {
                    iTotal[0] += Number(aaData[i][2]);
                    iTotal[1] += Number(aaData[i][3]);
                    iTotal[2] += Number(aaData[i][5]);
                    iTotal[3] += Number(aaData[i][6]);
                }
...
$j("#export").click(function () {
    expTable(document.getElementById("cashierReportTable"));
});