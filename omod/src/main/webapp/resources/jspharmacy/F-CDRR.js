...
var ooTable;
var aData;
$j("#FCDRRForm").validate();
$j("#FCDRRDateFrom").datepicker();
$j("#FCDRRDateTo").datepicker();
$j("form#FCDRRForm").submit(function () {
    if ($j("#FCDRRForm").valid()) {
        ooTable = $j('#FCDRRTable').dataTable({
            bJQueryUI:true,
            bRetrieve:true,
            bServerSide:true,
            bAutoWidth:false,
            bProcessing:true,
            sAjaxSource: "FCDRRProperties.form?" + "dateFrom=" + $j('#FCDRRDateFrom').val() + "&dateTo=" + $j('#FCDRRDateTo').val(),
...
$j("#export").click(function () {
    expTable(document.getElementById("FCDRRTable"));
});