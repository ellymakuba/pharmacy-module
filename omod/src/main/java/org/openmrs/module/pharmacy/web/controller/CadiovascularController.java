...
public class CadiovascularController {

    private static final Log log = LogFactory.getLog(CadiovascularController.class);
    ...
    private String receiptNo,waiverNo,dQuantity,totalAmount,totalWaived,expectedTotal;
    private boolean aded=false;


    @Authorized("Manage Pharmacy")
    @RequestMapping(method = RequestMethod.GET, value = "module/pharmacy/cadiovascularProcessor")
    public synchronized void pageLoad(ModelMap map) {

    }

    @RequestMapping(method = RequestMethod.POST, value = "module/pharmacy/cadiovascularProcessor")
...
//
                    ...
                    else if (value.substring(0, value.indexOf("*")).equalsIgnoreCase("ObsSig"))
                    {
                        ...
                        medicationProcessor.setNumberPerPack(value.substring(value.indexOf("#") + 1, (value.indexOf("$"))));
                        medicationProcessor.setDailyDose(value.substring(value.indexOf("@") + 1, (value.indexOf(",")- 1)));
                    ...
                    else if (value.substring(0, value.indexOf("*")).equalsIgnoreCase("PAID")) {
                        RequestedFirstPass = false;
                        medicationProcessor = new MedicationProcessor();
                        medicationProcessor.setConcept(value.substring(value.indexOf("|") + 1, (value.indexOf("#"))));
                        medicationProcessor.setConceptAnswer(value.substring(value.indexOf("@") + 1, (value.length() - 1)));
                    ...
                    if (value.substring(0, value.indexOf("@")).equalsIgnoreCase("Days")) {
                        ...
                        if(listMedicationProcessors.size()>1){
                            medicationProcessor =listMedicationProcessors.get(1);
                            if(value.substring(value.indexOf("@") + 1, (value.length() - 1)).length()>0){
                                medicationProcessor.setDispensedQuantity(value.substring(value.indexOf("@") + 1, (value.length() - 1)));
                    ...
                    else if (value.substring(0, value.indexOf("@")).equalsIgnoreCase("DailyDose1")) {
                        if(aded) {
                            if(listMedicationProcessors.size()>1){
                                medicationProcessor =listMedicationProcessors.get(1);
                                medicationProcessor.setDailyDose(value.substring(value.indexOf("@") + 1, (value.length() - 1)));
                    ...
                    else if (value.substring(0, value.indexOf("@")).equalsIgnoreCase("rno")) {
//                        if(addToBigList)          {
...
//                        if(addToBigList)          {
                    ...
                    else if (value.substring(0, value.indexOf("@")).equalsIgnoreCase("TOTALAMOUNT")) {
//                        if(addToBigList)          {
                        totalAmount=  value.substring(value.indexOf("@") + 1, (value.length() - 1));


                    }
                    else if (value.substring(0, value.indexOf("@")).equalsIgnoreCase("EXPECTEDTOTAL")) {
//                        if(addToBigList)          {
                        expectedTotal=  value.substring(value.indexOf("@") + 1, (value.length() - 1));


                    }
                    else if (value.substring(0, value.indexOf("@")).equalsIgnoreCase("TOTALWAIVED")) {
//                        if(addToBigList)          {
                        totalWaived=  value.substring(value.indexOf("@") + 1, (value.length() - 1));
                    }

                    else if (value.substring(0, value.indexOf("@")).equalsIgnoreCase("Packs")) {
                        medicationProcessor = new MedicationProcessor();
                        medicationProcessor.setPackets(value.substring(value.indexOf("@") + 1, (value.length() - 1)));
                        listMedicationProcessors.add(medicationProcessor);
//                        }
...
//          pharmacyEncounter.setEncounter(service.getPharmacyEncounterTypeByName(encounterProcessor.getEncounterType()));
            ...
            while (v.hasNext()) {
                ...
                drugExtra.setExpected_amount(CheckIfDoubleNull(expectedTotal));
                ...
                int g = CheckIfIntNull(c.get(1).getDispensedQuantity());
                //log.info("quantity dispensed is+++++++++++++++++++++++++++++++++++++++++++++++++"+c.get(1).getDispensedQuantity());
                pharmacyDrugOrder.setQuantityGiven(g);
                pharmacyDrugOrder.setUnits(null);
                numbersInventtory[position][0]= c.get(0).getDrug().getDrugId();
                numbersInventtory[position][1]= g;
                ...
                listAnotherPharmacyObs.add(createPharmacyObs(c.get(4).getConcept(), c.get(4).getConceptAnswer(), "", c.get(0).getDrug().getDrugId().toString(), encounterProcessor.getPrescriber(), locationVal, encounterProcessor.getPatientId(), pharmacyEncounter, null,"DrugFrequency"));
