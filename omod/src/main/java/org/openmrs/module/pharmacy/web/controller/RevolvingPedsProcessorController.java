...
public class RevolvingPedsProcessorController {
    ...
    public String receiptNo,waiverNo;

    @Authorized("Manage Pharmacy")
    @RequestMapping(method = RequestMethod.GET, value = "module/pharmacy/revolvingPedsProcessor")
    public synchronized void pageLoad(ModelMap map) {

    }

    @RequestMapping(method = RequestMethod.POST, value = "module/pharmacy/revolvingPedsProcessor")
        ...
        try {
            ...
            for (int i = 0; i < array.size(); i++) {
                ...
                if (value.contains("*")) {
                    ...
                    if (value.substring(0, value.indexOf("*")).equalsIgnoreCase("Enc2")) {
                    ...
                    } else if (value.substring(0, value.indexOf("*")).equalsIgnoreCase("Units")) {
                        ...
                        medicationProcessor.setUnits(value.substring(value.indexOf("@") + 1, (value.length() - 1)));
                         listMedicationProcessors.add(medicationProcessor);
                    ...
                    } else if (value.substring(0, value.indexOf("@")).equalsIgnoreCase("Dosing")) {

                        medicationProcessor = new MedicationProcessor();
                          medicationProcessor.setDosingCalculation(value.substring(value.indexOf("@") + 1, (value.length() - 1)));
                        listMedicationProcessors.add(medicationProcessor);

                    } else if (value.substring(0, value.indexOf("@")).equalsIgnoreCase("Days")) {
...
//            pharmacyEncounter.setEncounter(service.getPharmacyEncounterTypeByName(encounterProcessor.getEncounterType()));
            ...
            pharmacyEncounter.setNextVisitDate(endDate);
               ...
               pharmacyEncounter.setDuration(Integer.parseInt(encounterProcessor.getDuration().trim()));
            ...
            while (v.hasNext()) {
                ...
                drugExtra.setAmount(CheckIfDoubleNull(c.get(5).getConceptAnswer()));
                drugExtra.setAmountw(CheckIfDoubleNull(c.get(6).getConceptAnswer()));
                ...
                pharmacyDrugOrder.setFrequency(CheckIfStringNull(c.get(2).getConceptAnswer()));
                ...
                listAnotherPharmacyObs.add(createPharmacyObs(c.get(2).getConcept(), c.get(2).getConceptAnswer(), "", c.get(0).getDrug().getDrugId().toString(), encounterProcessor.getPrescriber(), locationVal, encounterProcessor.getPatientId(), pharmacyEncounter, null,"DrugFrequency"));
               ...
               // listAnotherPharmacyObs.add(createPharmacyObs(c.get(5).getConcept(), c.get(5).getConceptAnswer(), "", c.get(0).getDrugId(), encounterProcessor.getPrescriber(), locationVal, encounterProcessor.getPatientId(), pharmacyEncounter, pharmacyOrders,"DrugPrice"));
                position++;
            }
